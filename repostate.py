...
import atexit # 🆕 Import: สำหรับ Graceful Shutdown
...
def get_local_ip() -> str:
    ...
    try:
        ...
        s.connect(("8.8.8.8", 80)) # Try connecting to an external server (Google DNS)
        ip = s.getsockname()[0]
        s.close()
        return ip
    except Exception:
...
def load_worker_config(key: str, default_val: str) -> str:
    """Loads worker config (Rname, Zone, Pass) from set-miner/miner.json."""
    ...
    if os.path.exists(config_path):
        try:
            ...
            pass # Return default on error

    return default_val

def generate_random_sols() -> float:
    """Generates a random simulated hashrate (Sols/s) for online worker."""
    return random.uniform(2000000.0, 3000000.0) 

# 🆕 ฟังก์ชันใหม่: ส่งสถานะ OFFLINE เมื่อโปรแกรมปิดตัวลง
def send_offline_report(worker_name: str, worker_tags: str, worker_pass: str):
    ...
    try:
        ...
        # Fails silently if server connection is already lost
        pass

def send_report(worker_name: str, worker_tags: str, worker_pass: str) -> bool:
    """Sends the worker status report (simulated) to the monitoring server."""
    global REPORT_URL, REPORT_FAILURE_COUNT
    
    # 1. จำลองสถานะปัจจุบัน
    stats = {
